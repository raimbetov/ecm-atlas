#!/usr/bin/env python3
"""
Quantile Normalization with Age Preservation - Agent claude_2
Strategy: Apply quantile normalization WITHIN age groups separately
This preserves Age signal while removing batch effects
"""
...
def quantile_normalize(df_matrix):
    """
    Perform quantile normalization on a dataframe.
    Columns are samples, rows are features.
    """
    # Sort each column
    ranked = df_matrix.rank(method='first')

    # Calculate mean of each rank across all columns
    sorted_df = np.sort(df_matrix.values, axis=0)
    means = sorted_df.mean(axis=1)

    # Replace values with the mean of that rank
    normalized = df_matrix.copy()
    for col in df_matrix.columns:
        ranks = ranked[col].values - 1  # Convert to 0-indexed
        normalized[col] = means[ranks.astype(int)]
...
print("\nApplying quantile normalization per tissue & age...")
# Apply quantile normalization within each tissue compartment AND age group
df_corrected = []
...
for tissue in df_long['Tissue_Compartment'].unique():
    ...
    # Process each age group separately - KEY INNOVATION
    for age in ['Young', 'Old']:
        ...
        # Pivot to wide format (proteins × studies)
        ...
        # Apply quantile normalization across studies
        # This harmonizes distributions WITHIN same age group
        normalized = quantile_normalize(wide)
