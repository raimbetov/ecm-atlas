#!/usr/bin/env python3
"""
Step 3: Filter ECM and convert to LEGACY format for Dipali 2023
"""
...
def main():
    ...
    df_annotated = pd.read_csv("Dipali_2023_long_annotated.csv")
    ...
    # Aggregate Young samples (Dipali: Age = 2.25 months)
    # Count non-NaN values per protein = N_Profiles
    df_young = df_ecm[df_ecm['Age'] == 2.25].groupby('Protein_ID').agg({
        'Protein_Name': 'first',
        'Gene_Symbol': 'first',
    ...
    # Aggregate Old samples (Dipali: Age = 11 months)
    df_old = df_ecm[df_ecm['Age'] == 11].groupby('Protein_ID').agg({
        'Abundance': [
            ('mean', lambda x: x.mean(skipna=True)),
    ...
    # 2. Organ (from Tissue - for Dipali it's "Ovary")
    df_wide['Organ'] = 'Ovary'  # From config

    # 3. Compartment (for Dipali without compartments = same as Tissue)
    df_wide['Compartment'] = 'Ovary'  # No compartments
    ...
    # Log2 transform (only for non-NaN and > 0)
    df_wide['Abundance_Young_transformed'] = df_wide['Abundance_Young'].apply(
        lambda x: np.log2(x) if pd.notna(x) and x > 0 else np.nan
    )
    df_wide['Abundance_Old_transformed'] = df_wide['Abundance_Old'].apply(
        lambda x: np.log2(x) if pd.notna(x) and x > 0 else np.nan
    )
    ...
    output_file = "Dipali_2023_LEGACY_format.csv"
