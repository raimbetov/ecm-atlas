#!/usr/bin/env python3
"""
Step 2: ECM Annotation for LiDermis 2021
...
def main():
    ...
    # Load long format
    df_long = pd.read_csv("LiDermis_2021_long_format.csv")
    log(f"Loaded long format: {len(df_long)} rows")
    log(f"Unique proteins: {df_long['Protein_ID'].nunique()}")

    # Load HUMAN matrisome (LiDermis uses human samples)
    matrisome_path = project_root / "references" / "human_matrisome_v2.csv"
    df_matrisome = pd.read_csv(matrisome_path)
    log(f"Loaded human matrisome: {len(df_matrisome)} ECM proteins")
    ...
    output_file = "LiDermis_2021_long_annotated.csv"
